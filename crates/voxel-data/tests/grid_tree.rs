...
mod tests {
	...
	fn tree_voxels(tree: &VoxelGridTree) -> HashMap<U16Vec3, u16> {
	...
	fn basic_insert_get_remove() {
		...
		assert_eq!(t.remove(&p(3, 4, 5)), Some(7));
	...
	fn root_promotion_preserves_existing_and_new_data() {
		let mut t = VoxelGridTree::new();
		assert_eq!(t.insert(&p(0, 0, 0), 1), None);
	...
	fn single_voxel_pair_build_uses_canonical_root_origin() {
		let mut t = VoxelGridTree::new();
		assert_eq!(t.insert(&p(5, 0, 0), 7), None);
		assert_eq!(t.insert(&p(8, 0, 0), 9), None);
		assert_eq!(t.get(&p(5, 0, 0)), Some(7));
		assert_eq!(t.get(&p(8, 0, 0)), Some(9));
	}
	...
	fn read_from_rejects_trailing_bytes() {
		...
		bytes.push(123);
		let result = VoxelGridTree::read_from(&mut Cursor::new(bytes));
		assert!(result.is_err(), "read_from accepted trailing bytes");
	}

	#[test]
	fn read_from_rejects_invalid_root_depth() {
		let result = VoxelGridTree::read_from(&mut Cursor::new(root_only_stream(255, 0)));
		assert!(result.is_err(), "read_from accepted an impossible root depth");
	}

	#[test]
	fn read_from_recomputes_item_count_from_contents() {
		let tree = VoxelGridTree::read_from(&mut Cursor::new(root_only_stream(0, 99))).expect("deserializing minimal tree");
