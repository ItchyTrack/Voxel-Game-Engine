...
use bevy::math::{I8Vec3, IVec2, IVec3, Quat, Vec3};
use bevy::prelude::*;
...
impl Grid {
	...
	fn local_of(sub_grid_pos: IVec3, pos: IVec3) -> UVec3 {
		(pos - sub_grid_pos).as_uvec3()
	}
	...
	fn read_region_into(out: &mut Voxels, out_min: IVec3, sub_grid_pos: IVec3, slot: &SubGridSlot, cell_lo: IVec3, cell_hi: IVec3) {
		...
		let read_region = crate::grid_tree::NonZeroVoxelRegion::from_min_size(region_lo.as_ivec3(), region_hi - region_lo).unwrap();
		for (pos, run, voxel) in slot.voxels.grid_tree().iter() {
			...
			let run_hi = (pos + UVec3::splat(run)).min(region_hi);
			let out_extent = run_hi - run_lo;
			if out_extent == UVec3::ZERO { continue; }
			let world = sub_grid_pos + run_lo.as_ivec3();
			areas.push(((world - out_min).as_uvec3(), out_extent, voxel));
		}
		out.add_areas(&areas);
	}
	...
	pub fn set_area(&mut self, min: IVec3, size: UVec3, voxel: Option<VoxelRef<'_>>) -> HashSet<IVec3> {
		...
		if let Some(voxel) = voxel {
			for (sub_grid_pos, cell_lo, cell_hi) in self.write_regions(min, hi) {
				...
				let extent = (cell_hi - cell_lo).as_uvec3();
...
mod tests {
	...
	fn splat_voxels_blocking_uses_tree_merge_for_empty_single_subgrid() {
		...
		src.add_area(UVec3::ZERO, UVec3::splat(16), vox(7).get_ref());
	...
	fn splat_voxels_reproduces_source() {
		...
		for x in 0..40 {
			for y in 0..40 {
				for z in 0..40 {
					src.add_voxel(UVec3::new(x, y, z), vox(1).get_ref());
				}
			}
		}
		for &(x, y, z, c) in &[(5, 60, 3, 2), (70, 1, 70, 3), (33, 33, 33, 4)] {
			src.add_voxel(UVec3::new(x, y, z), vox(c).get_ref());
		}
	...
	fn subgrid_bvh_stays_in_sync_after_blocking_splat() {
		bevy::tasks::ComputeTaskPool::get_or_init(|| bevy::tasks::TaskPoolBuilder::new().build());
		let mut src = Voxels::new_with_type(test_type_info());
		src.add_area(UVec3::ZERO, UVec3::splat(8), vox(7).get_ref());
		let mut grid = Grid::new_with_type(test_type_info());
