...
impl VoxelPalette {
	pub fn new() -> Self {
		Self { palette: BiHashMap::new(), next_id: 0 }
	}
...
impl Voxels {
	pub fn new() -> Self {
		Self { voxels: VoxelGridTree::new(), voxel_palette: VoxelPalette::new(), bounding_box: Mutex::new(None), bounding_box_dirty: AtomicBool::new(false) }
	}

	pub fn add_voxel(&mut self, pos: U16Vec3, voxel: Voxel) -> Option<Voxel> {
	...
	pub fn add_area(&mut self, pos: U16Vec3, size: U16Vec3, voxel: Voxel) { self.add_areas(&[(pos, size, voxel)]); }

	pub fn add_voxels(&mut self, voxels: &[(U16Vec3, Voxel)]) {
		...
		for (pos, voxel) in voxels {
			let id = if let Some((_, id)) = palette_cache.iter().find(|(cached, _)| cached == voxel) { *id } else {
				let id = self.voxel_palette.palette_id(voxel);
				palette_cache.push((*voxel, id));
				id
			};
			tree_voxels.push((*pos, id));
			bounds = Some(match bounds { Some((mn, mx)) => (mn.min(*pos), mx.max(*pos)), None => (*pos, *pos) });
		}
	...
	pub fn add_areas(&mut self, areas: &[(U16Vec3, U16Vec3, Voxel)]) {
	...
	pub fn add_palette_voxels(&mut self, voxels: &[(U16Vec3, u16)], source_palette: &VoxelPalette) {
		if voxels.is_empty() { return; }
	...
	pub fn add_palette_voxels_in_bounds(&mut self, voxels: &[(U16Vec3, u16)], source_palette: &VoxelPalette, min: U16Vec3, max: U16Vec3) {
		if voxels.is_empty() { return; }
		if self.is_empty() {
	...
	pub fn add_palette_areas(&mut self, areas: &[(U16Vec3, U16Vec3, u16)], source_palette: &VoxelPalette) {
		let mut tree_areas = Vec::with_capacity(areas.len());
		let mut bounds: Option<(U16Vec3, U16Vec3)> = None;
		if self.is_empty() {
			self.voxel_palette = source_palette.clone();
	...
	pub fn remove_voxel(&mut self, pos: &U16Vec3) -> Option<Voxel> {
	...
	pub fn voxel(&self, pos: &U16Vec3) -> Option<&Voxel> { self.voxel_palette.voxel(self.voxels.get(pos)?) }
