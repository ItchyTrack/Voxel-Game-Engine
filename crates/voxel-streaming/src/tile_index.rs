...
impl<K: TileIndexKey> TileIndex<K> {
	pub fn insert(&mut self, key: K) {
		...
		for_each_bin_intersecting_key(key, |bin| {
	...
	pub fn remove(&mut self, key: K) {
		for_each_bin_intersecting_key(key, |bin| {
			let Some(keys) = self.bins.get_mut(&(key.lod(), bin)) else { return };
	...
	pub fn for_each_overlapping(&self, min: IVec3, size: IVec3, max_lod: u8, skip_lod: Option<u8>, mut f: impl FnMut(K)) {
		let area_max = min + size;
		let mut seen = HashSet::new();
		let capped_max_lod = self.max_lod.min(max_lod);
		for lod in 0..=capped_max_lod {
			if skip_lod == Some(lod) {
				continue;
			}
	...
	pub fn keys_covering_point(&self, point: IVec3, min_lod: u8) -> Vec<K> {
		let mut out = Vec::new();
		let mut seen = HashSet::new();
		for lod in min_lod..=self.max_lod {
...
fn for_each_bin_intersecting_key<K: TileIndexKey>(key: K, f: impl FnMut(IVec3)) {
	for_each_bin_intersecting_region(key.min(), key.size(), key.lod(), f);
}
