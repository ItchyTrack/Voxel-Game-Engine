...
fn planet_chunk_mass_properties(tile: &PlanetTile, chunk: IVec3) -> MassProperties {
	...
	for_each_shape_column(tile, origin, 1, |x, y, z0, z1| {
		...
		properties = properties.add(vertical_run_mass_properties(
			FixedVec3::from(IVec3::new(voxel_x, voxel_y, voxel_z)), length,
		));
	});
...
fn vertical_run_mass_properties(origin: FixedVec3, count: u64) -> MassProperties {
	let mass = u64::from(PLANET_VOXEL_MASS) * count;
	let length = count as f64;
	MassProperties {
		mass: Mass(mass),
		center_of_mass: CenterOfMass(origin + FixedVec3::new(Fixed::from_num(0.5), Fixed::from_num(0.5), Fixed::from_num(count) / Fixed::from_num(2))),
...
fn for_each_shape_column(
	...
	if extent <= 4 {
		...
		let sample_base_z = Fixed::from_num(origin.z + sample_offset) + Fixed::from_num(0.5);
		if extent == 1 {
			let sample_x = Fixed::from_num(origin.x + sample_offset) + Fixed::from_num(0.5);
			let sample_y = Fixed::from_num(origin.y + sample_offset) + Fixed::from_num(0.5);
			if let Some((z0, z1)) = column_shape_z_range(tile, sample_x, sample_y, sample_base_z, 1, Fixed::from_num(step)) {
				visit(0, 0, z0, z1);
			}
			return;
		}
		for y in 0..extent {
			let sample_y = Fixed::from_num(origin.y + y * step + sample_offset) + Fixed::from_num(0.5);
			for x in 0..extent {
				let sample_x = Fixed::from_num(origin.x + x * step + sample_offset) + Fixed::from_num(0.5);
				if let Some((z0, z1)) = column_shape_z_range(
					tile, sample_x, sample_y, sample_base_z, extent, Fixed::from_num(step),
				) {
					visit(x, y, z0, z1);
				}
			}
...
fn for_each_shape_column_batched(
	...
	let sample_base_z = Fixed::from_num(origin.z + sample_offset) + Fixed::from_num(0.5);
	let z_index = |z: Fixed| {
		((z - sample_base_z) / Fixed::from_num(step)).ceil().clamp(Fixed::ZERO, Fixed::from_num(extent)).to_num::<i32>()
	};
	let slab_z0 = z_index(Fixed::from_num(-TILE_INWARD_DEPTH));
	let slab_z1 = z_index(Fixed::from_num(TILE_OUTWARD_HEIGHT));
	if slab_z0 >= slab_z1 { return; }

	let mut sample_x = [Fixed::ZERO; CHUNK_SIZE as usize];
	for (x, sample) in sample_x[..extent].iter_mut().enumerate() {
		*sample = Fixed::from_num(origin.x + x as i32 * step + sample_offset) + Fixed::from_num(0.5);
	}

	// Reject empty chunks using the furthest sampled corner of each halfspace.
	let sample_y0 = Fixed::from_num(origin.y + sample_offset) + Fixed::from_num(0.5);
	let sample_y1 = Fixed::from_num(origin.y + (extent as i32 - 1) * step + sample_offset) + Fixed::from_num(0.5);
	for h in &tile.halfspaces {
		let x = if h.normal.x >= Fixed::ZERO { sample_x[extent - 1] } else { sample_x[0] };
		let y = if h.normal.y >= Fixed::ZERO { sample_y1 } else { sample_y0 };
		let base = h.normal.x * x + h.normal.y * y + h.offset;
		if h.normal.z > Fixed::from_num(1e-6) {
			if z_index((-TILE_SHAPE_EPSILON - base) / h.normal.z) >= slab_z1 { return; }
		} else if h.normal.z < Fixed::from_num(-1e-6) {
	...
	'rows: for y in 0..extent {
		let sample_y = Fixed::from_num(origin.y + y as i32 * step + sample_offset) + Fixed::from_num(0.5);
		let mut min_z = [Fixed::from_num(-TILE_INWARD_DEPTH); CHUNK_SIZE as usize];
		let mut max_z = [Fixed::from_num(TILE_OUTWARD_HEIGHT); CHUNK_SIZE as usize];
		...
		for h in &tile.halfspaces {
			...
			if h.normal.z > Fixed::from_num(1e-6) {
				...
				for (max_z, &x) in max_z[..extent].iter_mut().zip(&sample_x[..extent]) {
					let base = h.normal.x * x + y_term + h.offset;
					if base < -TILE_SHAPE_EPSILON { *max_z = Fixed::from_num(-TILE_INWARD_DEPTH); }
				}
			}
		}
...
pub(super) fn column_shape_z_range(
	...
	let mut min_sample_z = Fixed::from_num(-TILE_INWARD_DEPTH);
	let mut max_sample_z = Fixed::from_num(TILE_OUTWARD_HEIGHT);
	...
	let z0 = ((min_sample_z - sample_base_z) / step).ceil().clamp(Fixed::ZERO, Fixed::from_num(extent_z)).to_num::<i32>();
	let z1 = ((max_sample_z - sample_base_z) / step).ceil().clamp(Fixed::ZERO, Fixed::from_num(extent_z)).to_num::<i32>();
