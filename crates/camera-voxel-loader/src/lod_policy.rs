...
impl DesiredSourceDelta {
	fn empty() -> Self {
		Self { tiles: SetDelta { added: Vec::new(), removed: Vec::new() } }
	}
}
...
#[allow(dead_code)]
pub(crate) fn add_near_tiles(
	out: &mut HashSet<TileKey>, grid: GridId, center: IVec3, camera_voxel_loader: &CameraVoxelLoader, streaming: &GridStreaming,
) {
	add_near_tiles_for_settings(out, grid, center, &camera_voxel_loader.settings, streaming);
}

fn add_near_tiles_for_settings(
	out: &mut HashSet<TileKey>, grid: GridId, center: IVec3, settings: &CameraVoxelLoaderSettings, streaming: &GridStreaming,
) {
...
#[allow(dead_code)]
pub(crate) fn add_lod_tiles(
	out: &mut HashSet<TileKey>, grid: GridId, center: IVec3, camera_voxel_loader: &CameraVoxelLoader, streaming: &GridStreaming,
) {
	add_lod_tiles_for_settings(out, grid, center, &camera_voxel_loader.settings, streaming);
}

fn add_lod_tiles_for_settings(
	out: &mut HashSet<TileKey>, grid: GridId, center: IVec3, settings: &CameraVoxelLoaderSettings, streaming: &GridStreaming,
) {
	if streaming.presence().len() == 0 {
		return;
	}
...
fn add_all_tiles_for_settings(
	out: &mut HashSet<TileKey>, grid: GridId, center: IVec3, settings: &CameraVoxelLoaderSettings, streaming: &GridStreaming,
) {
	add_near_tiles_for_settings(out, grid, center, settings, streaming);
	add_lod_tiles_for_settings(out, grid, center, settings, streaming);
}
...
pub(crate) fn is_tile_wanted(settings: &CameraVoxelLoaderSettings, streaming: &GridStreaming, center: IVec3, key: TileKey) -> bool {
	wants_tile(settings, streaming, center, key)
}

pub(crate) fn update_desired_sources_delta(
	desired_tiles: &mut HashSet<TileKey>, grid: GridId, previous_center: Option<IVec3>, new_center: IVec3, settings_changed: bool,
	...
	let mut delta = if settings_changed || previous_center.map_or(true, |old| should_rebuild_policy(old, new_center)) {
		rebuild_desired_sources_delta(desired_tiles, grid, new_center, settings, streaming)
	} else if let Some(old_center) = previous_center.filter(|old| *old != new_center) {
		DesiredSourceDelta { tiles: update_tiles_delta(desired_tiles, grid, old_center, new_center, settings, streaming) }
	} else {
		DesiredSourceDelta::empty()
	};

	let removed_absent: Vec<_> =
		desired_tiles.iter().copied().filter(|key| key.grid == grid && !tile_has_present_chunk(streaming, key.min, 1i32 << key.lod)).collect();
...
fn rebuild_desired_sources_delta(
	...
	add_all_tiles_for_settings(&mut new_tiles, grid, center, settings, streaming);
...
pub(crate) fn update_tiles_delta(
	out: &mut HashSet<TileKey>, grid: GridId, old_center: IVec3, new_center: IVec3, settings: &CameraVoxelLoaderSettings, streaming: &GridStreaming,
) -> SetDelta<TileKey> {
	let mut candidates = HashSet::new();
	let (old_min, old_max) = near_bounds(old_center, settings);
	let (new_min, new_max) = near_bounds(new_center, settings);
	for_each_changed_slab(old_min, old_max - IVec3::ONE, new_min, new_max - IVec3::ONE, |min, max, _entering| {
		add_tiles_intersecting_box(&mut candidates, grid, 0, min, max, 1);
	});
	...
	for lod in 1..=settings.max_lod {
		...
		add_changed_tile_slabs(
			...
			lod,
			old_outer_min,
			old_outer_max - IVec3::ONE,
			new_outer_min,
			new_outer_max - IVec3::ONE,
			tile_size,
		);
		add_changed_tile_slabs(
			&mut candidates,
			...
			lod,
			old_inner_min,
			old_inner_max - IVec3::ONE,
			new_inner_min,
			new_inner_max - IVec3::ONE,
			tile_size,
		);
	}
	...
	for key in candidates {
		let old_wanted = wants_tile(settings, streaming, old_center, key);
		let new_wanted = wants_tile(settings, streaming, new_center, key);
...
fn wants_tile(settings: &CameraVoxelLoaderSettings, streaming: &GridStreaming, center: IVec3, key: TileKey) -> bool {
	if key.lod == 0 {
		let (min, max) = near_bounds(center, settings);
		return key.min.cmpge(min).all() && key.min.cmplt(max).all() && streaming.presence().is_present(key.min);
	}
...
fn add_changed_tile_slabs(
	out: &mut HashSet<TileKey>, grid: GridId, lod: u8, old_min: IVec3, old_max: IVec3, new_min: IVec3, new_max: IVec3, tile_size: i32,
) {
