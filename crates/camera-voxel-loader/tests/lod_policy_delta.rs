...
fn lod_policy_must_not_assign_multiple_lod_tiles_to_same_present_chunk() {
	...
	let chunk = ChunkKey { grid, chunk: IVec3::new(-15, 6, 11) };
	let lod2_tile = TileKey { grid, lod: 2, min: IVec3::new(-16, 4, 8) };
	let lod3_tile = TileKey { grid, lod: 3, min: IVec3::new(-16, 0, 8) };

	let mut streaming = GridStreaming::default();
	streaming.presence_mut().mark_present(chunk.chunk);

	let mut desired_chunks = HashSet::<ChunkKey>::new();
	let mut desired_tiles = HashSet::<TileKey>::new();
	add_near_chunks(&mut desired_chunks, grid, center, &controller, &streaming);
	add_lod_tiles(&mut desired_tiles, grid, center, &controller, &streaming);

	let covering_tiles: Vec<_> = desired_tiles
		.iter()
		.copied()
		.filter(|tile| tile.grid == chunk.grid && chunk.chunk.cmpge(tile.min).all() && chunk.chunk.cmplt(tile.min + tile.size()).all())
		.collect();
		...
		for x in min.x..max.x {
			for y in min.y..max.y {
				for z in min.z..max.z {
					let chunk = ChunkKey { grid, chunk: IVec3::new(x, y, z) };
					if !streaming.presence().is_present(chunk.chunk) {
						continue;
					}
					assert!(
	...
	for settings in explicit_settings_cases() {
		...
		for center in centers {
			...
			for &present in &present_chunks {
				let chunk = ChunkKey { grid, chunk: present };
...
#[test]
fn lod_policy_generated_tiles_match_is_lod_tile_wanted_for_explicit_settings() {
	for settings in explicit_settings_cases() {
		let grid: GridId = Entity::PLACEHOLDER;
		let controller = CameraVoxelLoader { settings: settings.clone() };
		let mut streaming = GridStreaming::default();
					...
					for y in (min.y..=max.y).step_by(tile_size as usize) {
						for z in (min.z..=max.z).step_by(tile_size as usize) {
							...
							assert_eq!(
								...
								is_lod_tile_wanted(&settings, &streaming, center, tile),
								"settings={settings:?}, center={center:?}: add_lod_tiles disagrees with is_lod_tile_wanted for {tile:?}"
							);
						}
					}
				}
...
fn incremental_lod_policy_matches_full_rebuild_for_deterministic_flight_stress() {
	...
	for step in 0..48 {
		...
		let delta = IVec3::new(
			(seed as i32).rem_euclid(5) - 2,
			((seed >> 8) as i32).rem_euclid(3) - 1,
			((seed >> 16) as i32).rem_euclid(5) - 2,
		);
...
fn covering_lod_tiles(desired_tiles: &HashSet<TileKey>, chunk: ChunkKey) -> Vec<TileKey> {
	desired_tiles
		.iter()
		.copied()
		.filter(|tile| tile.grid == chunk.grid && chunk.chunk.cmpge(tile.min).all() && chunk.chunk.cmplt(tile.min + tile.size()).all())
...
fn test_lod_inner_bounds(center: IVec3, settings: &CameraVoxelLoaderSettings, lod: u8) -> (IVec3, IVec3) {
	if lod == 1 { test_near_bounds(center, settings) } else { test_lod_outer_bounds(center, settings, lod - 1) }
}
...
fn test_lod_outer_bounds(center: IVec3, settings: &CameraVoxelLoaderSettings, lod: u8) -> (IVec3, IVec3) {
	...
	if lod < settings.max_lod { test_align_bounds_to_tile(bounds.0, bounds.1, 1i32 << (lod + 1)) } else { bounds }
}

fn test_near_bounds(center: IVec3, settings: &CameraVoxelLoaderSettings) -> (IVec3, IVec3) {
	test_align_bounds_to_tile(
		center - IVec3::splat(settings.near_radius_chunks),
		center + IVec3::splat(settings.near_radius_chunks + 1),
		2,
	)
}
...
fn covered_chunks(desired_chunks: &HashSet<ChunkKey>, desired_tiles: &HashSet<TileKey>) -> HashSet<ChunkKey> {
		...
		for x in tile.min.x..max.x {
			for y in tile.min.y..max.y {
				for z in tile.min.z..max.z {
					covered.insert(ChunkKey { grid: tile.grid, chunk: IVec3::new(x, y, z) });
				}
			}
		}
	}
