...
Lighting defaults to Nothing. **GI rays per face** selects 8, 16, 32, or 64 rays, defaulting to 32. The GI dropdown is enabled only for indirect lighting. Direct and indirect modes include shadows; there is no separate shadows toggle. Nothing and Direct retain the old 0.25 ambient fill. GI uses sampled sky and bounced sunlight instead; the fill is never added to contributor radiance.

**Direct rays per face** selects 1, 4, or 9 rays, defaulting to 1. It applies to Direct and indirect lighting, including contributor faces and overflow fallback. Samples use fixed 1×1, 2×2, or 3×3 grids across each face. Averaging their visibility smooths partial shadows; the sun direction stays fixed. This setting needs no extra arena memory and does not affect Nothing.
...
- Flat per-face lighting, no filtering or history. More GI rays reduce noise but do not add bounces or increase light strength.
...
- At full face capacity, the arena uses about 104, 136, 200, or 328 MiB per view for 8, 16, 32, or 64 rays. Smaller device limits reduce face capacity. Buffers remain allocated when lighting is disabled; changing GI ray count rebuilds them.
- Capacity or bounded-hash-probe failure falls back to Direct with ambient fill for the whole frame. This can occur as the camera angle changes the face count. A warning appears outside the collapsed stats panel. Fallback uses the same face sample positions and ray count, including faces reconstructed by AA, but may repeat work per pixel.
...
The smoke example saves images under the system temporary directory. It checks sun-normal shading, ambient fill, all GI and direct ray counts, partial shadows, color bleed, repeated frames, two-dimensional dispatches, AA, render-target resizing, and grid transforms. The overflow run checks that fallback matches Direct with ambient byte-for-byte for every direct ray count, with AA and grid transforms. The `--app` run captures the normal application in all modes, including 64-ray GI with nine direct rays and AA. The `--angles` diagnostic captures a camera sweep at 1686×948 and logs overflow flags.
