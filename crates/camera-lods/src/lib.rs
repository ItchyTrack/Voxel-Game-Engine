...
pub use grid_control::CameraLodGridControl;
pub use policy::{CameraLodPolicy, CameraLodTarget, FreezeCameraLods, apply_camera_lod_policy, update_camera_lod_policy};
...
impl Plugin for CameraLodsPlugin {
    fn build(&self, app: &mut App) {
        ...
        app.init_resource::<FreezeCameraLods>()
            .configure_sets(Update, CameraLodSet::Policy)
...
fn ensure_camera_lod_components(mut commands: Commands, cameras: Query<Entity, (With<Camera>, Added<Camera>)>) {
    for entity in cameras.iter() {
        commands.entity(entity).insert((LodRequestMap::default(), CameraLodGridControl::default(), CameraLodPolicy::default()));
    }
}
