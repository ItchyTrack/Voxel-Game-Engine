...
use voxel_sources::{CancellationToken, LodKey, SourceHandle};
...
impl PendingVoxelLoad {
	fn is_cancelled(&self) -> bool {
		match self {
			...
			Self::Lod(pending) => pending.cancellation.is_cancelled(),
		}
	}
}
...
pub(crate) struct ClientLoadRegistry {
	...
	lod_ids: HashMap<PendingLodKey, VoxelLoadId>,
...
impl ClientLoadRegistry {
	...
	pub fn request_lod(
		...
		if let Some(previous) = self.lod_ids.get(&pending_key).copied() {
			self.finish(previous, VoxelLoadOutcome::Cancelled);
		}

		let id = self.allocate_id();
		self.pending.insert(id, PendingVoxelLoad::Lod(PendingLod {
			key: pending_key,
			request_generation,
			deferred_generation: None,
			cancellation,
		}));
		self.lod_ids.insert(pending_key, id);
		self.requests.push_back(VoxelLoadRequest { id, kind: VoxelLoadRequestKind::Lod { grid, key, priority } });
	}
	...
	pub fn receive_chunk_changed(&mut self, handle: &SourceHandle, event: RemoteChunkChanged) {
					...
					match event.kind {
						RemoteChunkChangeKind::Changed { .. } => {
							if let Some(id) = self.chunk_ids.get(&key) {
								...
								pending.deferred_generation = Some(
									pending.deferred_generation.map_or(generation, |current| current.max(generation)),
								);
							} else {
		...
		for voxel_load in self.pending.values_mut() {
			let PendingVoxelLoad::Lod(pending) = voxel_load else { continue };
			if pending.key.grid != event.grid || !lod_overlaps_area(pending.key.key, event.min, event.size) {
				continue;
			}
			pending.deferred_generation = Some(
				pending.deferred_generation.map_or(generation, |current| current.max(generation)),
	...
	pub fn receive_response(&mut self, handle: &SourceHandle, from: impl std::fmt::Debug, response: &mut VoxelLoadResponse) {
		match &mut response.kind {
			...
			VoxelLoadResponseKind::Lod { grid, key, generation, voxels } => {
				self.receive_lod_response(handle, from, response.id, *grid, *key, *generation, voxels);
			}
		}
	}
	...
	fn receive_chunk_response(
		...
		if pending.deferred_generation.is_some_and(|dirty_generation| generation < dirty_generation) {
			handle.claim(grid, chunk, IVec3::ONE);
		}
		handle.loaded(grid, chunk, pending.request_generation, voxels);
	}

	fn receive_lod_response(
		&mut self,
		handle: &SourceHandle,
		from: impl std::fmt::Debug,
		id: VoxelLoadId,
		...
		let Some(pending) = self.pending.get(&id) else { return };
		let PendingVoxelLoad::Lod(pending) = pending else {
		...
		if pending.key.grid != grid || pending.key.key != key {
			warn!(?id, ?grid, min=?key.min, size=?key.size, lod=key.lod, ?from, "ignoring mismatched remote lod response");
			return;
		}
		...
		let Some(PendingVoxelLoad::Lod(pending)) = self.finish(id, VoxelLoadOutcome::Received) else {
			unreachable!("checked lod voxel load disappeared")
		};
		if pending.deferred_generation.is_some_and(|dirty_generation| generation < dirty_generation) {
			// Preserve the stale generation; voxel-streaming will reject it.
		}
		handle.loaded_lod(grid, key.min, key.size, key.lod as f32, pending.request_generation, voxels);
	}
	...
	fn finish(&mut self, id: VoxelLoadId, outcome: VoxelLoadOutcome) -> Option<PendingVoxelLoad> {
		...
		match &pending {
			...
			PendingVoxelLoad::Lod(pending) if self.lod_ids.get(&pending.key) == Some(&id) => {
				self.lod_ids.remove(&pending.key);
			}
			_ => {}
		}
...
mod tests {
	...
	#[test]
	fn cancelled_lod_has_one_terminal_outcome() {
		let cancellation = CancellationToken::new();
		let key = LodKey { min: IVec3::ZERO, size: IVec3::ONE, lod: 1 };
		let mut loads = ClientLoadRegistry::default();
		loads.request_lod(Entity::PLACEHOLDER, key, 0.0, 7, cancellation.clone());
		let request = loads.pop_request().unwrap();
