...
use crate::{TileData, TileGenerationContext, TileKey};
...
impl TileGenerationSession {
	...
	pub async fn receive_merged_voxels(&mut self, area: VoxelArea) -> Option<VoxelAreaResult> {
		...
		while let Some(result) = self.receive_voxels().await {
			...
			let offset = ((result.area.min - area.min) * self.chunk_size).div_euclid(IVec3::splat(step));
