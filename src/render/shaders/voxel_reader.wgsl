...
fn voxel_reader_palette_index(voxels_base: u32, voxel_data_index: u32, voxel_index: u32) -> u32 {
	let palette_size = voxel_reader_u8(voxels_base);
	return voxel_reader_u8(voxels_base + VOXELS_HEADER_SIZE + palette_size * 4 + voxel_data_index * VOXELS_NODE_INDEX_STEP + voxel_index);
}
fn voxel_reader_palette_color(voxels_base: u32, voxel_data_index: u32, voxel_index: u32) -> vec4<f32> {
	let palette_size = voxel_reader_u8(voxels_base);
	let palette_index = voxel_reader_u8(voxels_base + VOXELS_HEADER_SIZE + palette_size * VOXELS_COLOR_BYTES + voxel_data_index * VOXELS_NODE_INDEX_STEP + voxel_index);
