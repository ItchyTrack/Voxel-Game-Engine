...
impl AudioEngine {
	pub fn new() -> Self {
		...
		let supported_config = device.default_output_config().expect("No default audio output config available");
		...
		T: Sample + SizedSample + FromSample<f32>,
	{
		...
		let mut synth = SineSynth::new(config.sample_rate.0 as f32, 80.0, 0.1);
