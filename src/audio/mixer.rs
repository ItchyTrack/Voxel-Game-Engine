        ...
        T: Sample + SizedSample + FromSample<f32>,
    {
        ...
        for frame in output.chunks_mut(channels) {
            ...
            for voice in &mut self.active_voices {
                ...
                let (voice_left, voice_right) = self.listener.stereo_gains(voice.position, voice.gain);
...
impl ListenerState {
    fn stereo_gains(&self, emitter_position: Vec3, source_gain: f32) -> (f32, f32) {
        ...
        let pan = direction.dot(right).clamp(-1.0, 1.0);
        let angle = (pan + 1.0) * std::f32::consts::FRAC_PI_4;
        let attenuation = source_gain / (1.0 + distance * 0.1);
